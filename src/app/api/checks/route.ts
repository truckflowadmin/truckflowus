...
/**
 * DELETE /api/checks?checkId=X — delete a voided manual check
 */
export async function DELETE(req: NextRequest) {
  ...
  if (existing.status !== 'VOID') {
    return NextResponse.json({ error: 'Only voided checks can be deleted' }, { status: 400 });
  }

  await (prisma as any).manualCheck.delete({ where: { id: checkId } });
