          ...
          {tripSheets.map((ts) => {
            ...
            return (
              <div key={ts.id} className="rounded-lg border border-steel-200 bg-white p-4">
                ...
                <div className="mt-3 pt-3 border-t border-steel-100 flex gap-2">
                  <a
                    href={`/api/driver/trip-sheet-pdf?sheetId=${ts.id}`}
                    download={`trip-sheet-${ts.id}.pdf`}
                    className="text-xs font-medium text-steel-600 bg-steel-100 hover:bg-steel-200 px-3 py-1.5 rounded-lg transition-colors inline-flex items-center gap-1"
                  >
                  ...
                  <button
                    onClick={() => {
                      const w = window.open(`/api/driver/trip-sheet-pdf?sheetId=${ts.id}`, '_blank');
                      if (w) setTimeout(() => w.print(), 1000);
                    }}
                    className="text-xs font-medium text-steel-600 bg-steel-100 hover:bg-steel-200 px-3 py-1.5 rounded-lg transition-colors hidden sm:inline-flex items-center gap-1"
                  >
