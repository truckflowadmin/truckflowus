...
async function updateBrokerAction(formData: FormData) {
  ...
  const broker = await prisma.broker.findFirst({ where: { id: brokerId, companyId } });
  ...
  redirect(`/sa/tenants/${companyId}/brokers`);
}

export default async function SuperadminEditBrokerPage({
  params,
}: {
  params: { id: string; brokerId: string };
}) {
  await requireSuperadmin();
  ...
  const broker = await prisma.broker.findFirst({
    where: { id: params.brokerId, companyId: company.id },
  });
  if (!broker) notFound();
    ...
    <div className="p-4 md:p-8 max-w-3xl">
      ...
      <form action={updateBrokerAction} className="panel-sa p-6 space-y-5">
        ...
        <div className="flex items-center gap-3 pt-2 border-t border-purple-500/30">
          ...
          <Link href={`/sa/tenants/${company.id}/brokers`} className="text-sm text-purple-400 hover:text-purple-200">Cancel</Link>
        </div>
      </form>
